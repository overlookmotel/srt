 ...
 * @param {number} [options.minMsPerChar] - Minimum milliseconds per character
 * @returns {Object}
 ...
 */
function conform(subtitles, options) {
	// Conform options
	const minMsPerChar = (options || {}).minMsPerChar || 0;
	...
	subtitles.forEach((subtitle, index) => {
		...
		// Remove HTML except for italics
		runConform(
			t => t.replace(/(<[^\n>]+>)/gu, whole => (['<i>', '</i>'].includes(whole) ? whole : '')),
			'HTML tags'
		);

		// Remove SRT formatting tags
		runConform(t => t.replace(/\{\\an[^}\n]*\}/gu, ''), 'SRT formatting tags');

		// Convert '&nbsp;' to space
		runConform(t => t.replace(/&nbsp;/gu, ' '), "'&nbsp;' HTML");
		...
		// NB Will already have beem caught by `parse()` except if tabs, HTML, SRT formatting
		// or '&nbsp;' is only content
		if (text === '') {
			text = '_';
		...
		if (minMsPerChar) {
			const minDuration = Math.round(minMsPerChar * text.length);
			if (duration < minDuration) {
				end = start + minDuration;
				const cannotFullyFix = (nextStart !== null && end > nextStart);
				if (cannotFullyFix) end = nextStart;
				addProblem(
					`Subtitle duration ${duration} ms under ${minMsPerChar} ms/character (${text.length} chars)`,
					!cannotFullyFix
				);
				duration = end - start;
			}
 ...
 * @param {Array<Object>} subtitles - Array of subtitle objects (as created by `parse()`)
 * @param {Object} [options] - Options object (same as for `conform()`)
 * @returns {Array<Object>} - Array of problems found (same as `.problems` returned by `conform()`)
 */
function validate(subtitles, options) {
	const {problems} = conform(subtitles, options);
	return problems;
}
